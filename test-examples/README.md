...
| 08xx | directory concretes (`input/` trees, `.yo/` overlays) |
...
| `in.yo` | input parsed with `parseYamlover(src, uri)` |
...
| `input/` | a directory subtree (may contain a `.yo/` overlay) materialized with the engine's `walkDir(abs, { noGraft: true })` |
| `from` | one repo-relative path (e.g. `examples/06-tour.yo`) resolved against the repo root, then treated by the rules above — big human examples are referenced, not copied; the goldens still live here |
...
| `out.yo` | golden `serializeYamlover(doc)` output, asserted **byte-for-byte**; the harness also reparses it and asserts IR-equality with the input's parse |
| `error` | for erroneous fixtures, **instead of** `ir.json`/`out.yo`: one line, a JS regex matched against the thrown error's message |
...
fixture whose IR contains a blob **omits** `out.yo`; the harness verifies the omission
...
- Inputs are hand-authored first; `ir.json` / `out.yo` are generator-produced, then
