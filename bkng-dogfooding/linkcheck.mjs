// RETIRED as of ed2914a — kept as the origin story, do not reach for it.
//
// The product now holds THE LINK INVARIANT itself: dead prose links are `link/dead-target`
// warnings from the doctor sweep (server/link-check.ts), logged at startup and on every
// reconcile and listed by GET /api/doctor. That check is strictly better than this one —
// it also scans `.md` files, which this script skips (`endsWith('.yo')`), and that gap is
// exactly what hid the code-span/fence bug until the doctor found it. Use the server.
//
...
//
// Existed because nothing in the product reported a prose link that resolves to nothing —
