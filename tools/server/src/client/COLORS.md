...
emitted by `render.tsx`.

| Token | Class | Variable | Hex | Note |
|------|-------|----------|-----|------|
| Mapping / property key | `.k` | `--jb-key` | `#9876AA` | purple |
| String scalar | `.s` | `--jb-string` | `#6A8759` | green |
| Number scalar | `.n` | `--jb-number` | `#6897BB` | blue |
| `true` / `false` | `.b` | `--jb-keyword` | `#CC7832` | orange (keyword) |
| `null` / `~` | `.null` | `--jb-keyword` | `#CC7832` | orange (keyword) |
| Structural punctuation `:` `{}` `[]` | `.punct` | `--jb-punct` | `#A9B7C6` | default text |
| Comment / `!!binary` note | `.c` | `--jb-comment` | `#808080` | gray |
| YAML list dash `-` | `.yaml-dash` | `--jb-dash` | `#808080` | gray — matches the fold chevron |
