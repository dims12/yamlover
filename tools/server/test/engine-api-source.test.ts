...
describe("GET /api/source", () => {
  it("a document root answers the RAW body; a deeper node the re-serialized subtree", async () => {
    const root = tmpTree({ "note.yo": "a: 1\nb:\n  c: 2\n" });
    const h = createHandlers(root, { gitignore: false });
    await h.ready;
    expect(call(h, "/api/source", { path: ":note.yo" }).json).toEqual({ source: "a: 1\nb:\n  c: 2\n" });
    expect(call(h, "/api/source", { path: ":note.yo:b" }).json).toEqual({ source: "c: 2\n" });
    const bad = call(h, "/api/source", { path: ":note.yo:zzz" });
...
describe("root emplace with ENTRY facets — the whole-body flush", () => {
  it("replaces the body; the leading comments stand; the payload is validated first", async () => {
    const root = tmpTree({ "doc.yo": "# the lead comment\nold: 1\n" });
    const h = createHandlers(root, { gitignore: false });
    await h.ready;
    const file = path.join(root, "doc.yo");
    const ok = await callBody(h, "POST", "/api/edit", { path: ":doc.yo", op: "emplace", yamlover: "x: 1\ny: 2" });
    expect(ok.status).toBe(200);
    expect(fs.readFileSync(file, "utf8")).toBe("# the lead comment\nx: 1\ny: 2\n");
    const bad = await callBody(h, "POST", "/api/edit", { path: ":doc.yo", op: "emplace", yamlover: "x: [unclosed" });
