...
describe("/api/query", () => {
  it("evaluates a colon-grammar query and returns client paths", async () => {
    const h = createHandlers(
      tmpTree({ "team.yo": "alice:\n  age: 31\nbob:\n  age: 9\n" }),
      { gitignore: false },
    );
    await h.ready;
    const r = call(h, "/api/query", { q: ": team.yo: ?: age: >10" });
    expect(r.status).toBe(200);
    expect(r.json.results).toEqual([":team.yo:alice:age"]);
  });
  ...
  it("shape=tree returns TreeNode-shaped results", async () => {
    ...
    await h.ready;
    const r = call(h, "/api/query", { q: ": team.yo: ?", shape: "tree" });
    expect(r.status).toBe(200);
  ...
  it("shape=tree filters the hidden .yo overlay and still 400s a malformed query", async () => {
    const h = createHandlers(
      tmpTree({ name: "Alice", ".yo/settings.yo": "width: 80\n" }),
      { gitignore: false },
    );
    await h.ready;
    // `: ?` fans out over the root's children — the hidden `.yo` overlay must not appear.
    const r = call(h, "/api/query", { q: ": ?", shape: "tree" });
    expect(r.status).toBe(200);
  ...
  it("shape=filter returns the pruned ancestor tree with match flags", async () => {
    ...
    await h.ready;
    const r = call(h, "/api/query", { q: ": team.yo: ?: age: >10", shape: "filter" });
    expect(r.status).toBe(200);
    expect(r.json.matches).toEqual([":team.yo:alice:age"]);
    expect(r.json.truncated).toBe(false);
    const root = r.json.root;
    expect(root.path).toBe(":");
    expect(root.match).toBeUndefined(); // an ancestor, not a match
    // the pruned tree holds ONLY the ancestor chain — pets.yo and bob are absent
    expect(root.children.map((c: any) => c.path)).toEqual([":team.yo"]);
    const team = root.children[0];
    expect(team.children.map((c: any) => c.path)).toEqual([":team.yo:alice"]);
    const age = team.children[0].children[0];
    expect(age.path).toBe(":team.yo:alice:age");
  ...
  it("shape=filter: a match ships its real children one level deep (what lies below the path)", async () => {
    ...
    await h.ready;
    const r = call(h, "/api/query", { q: ": team.yo: alice", shape: "filter" });
    expect(r.status).toBe(200);
    expect(r.json.matches).toEqual([":team.yo:alice"]);
    const team = r.json.root.children[0];
    // siblings of the match's ancestors stay pruned (bob and pets.yo absent)
    expect(r.json.root.children.map((c: any) => c.path)).toEqual([":team.yo"]);
    expect(team.children.map((c: any) => c.path)).toEqual([":team.yo:alice"]);
    ...
    expect(alice.children.map((c: any) => c.path)).toEqual([":team.yo:alice:age", ":team.yo:alice:city"]);
  ...
  it("shape=filter: a match that is an ancestor of another match keeps both flagged", async () => {
    ...
    const alice = team.children[0];
    expect(alice.match).toBe(true);
    expect(alice.children[0].path).toBe(":team.yo:alice:age");
  ...
  it("the scope ladder is honored: `::` (project) searches the grafted taxonomy, `:` (document) does not", async () => {
    ...
    // and the `.yo` OVERLAY stays off search results at EVERY rung
    const overlay = call(h, "/api/query", { q: ":: ...: settings.yo", shape: "filter" });
