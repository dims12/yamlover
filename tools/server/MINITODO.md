...
- Tag picker: autocomplete over indexed tags (the path input + bare-name create-on-miss exist; search/completion doesn't — the evaluator is LIVE now: wire the picker to `GET /api/query` with `:: ...: !!<format: x-yamlover-onto>`-style queries)
...
- ~~/examples/51-dir-yo/name chooses marklower format by default without any reasons~~ — DONE (marklower is asked for by name; `chunkOf` stamps a chapter's inline chunks)
