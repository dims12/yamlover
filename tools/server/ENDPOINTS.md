...
| `/api/annotate` | POST / DELETE | POST `{ target, tag, description?, params? }`; DELETE `?target&tag` | Apply / remove a tag application (a membership bookmark on the target) |
| `/api/fragment` | POST | `{ target, selector, imageBase64? }` | Mark a fragment (a region) under the target's `.yo: fragments:` (legacy `yo:` reused), optional crop sidecar; returns `{ slug, fragmentPath }` |
