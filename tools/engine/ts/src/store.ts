...
const SCHEMA_VERSION = 13; // 13: `pattern: true` selector clauses + nested `members:` reaching subdirectories; 12: members/others schema clauses + meta `concrete:` decode; 11: parseError meta on degraded nodes — the write gate reads it; 10: a DIRECTORY-backed member is container shape (a titled childless subchapter folds to chapter); 9: anchored members route through the items schema; 8: !!yo exemption + the comment tail rule; 7: bare-integer segments + null keys (label_null); 6: dangling holder/label/pos/edge/external; 5: non-finite scalar tags; 4: colon-form paths
