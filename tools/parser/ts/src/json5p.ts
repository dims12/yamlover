...
//
// Adds over JSON5: `*'<pointer>'` (deref), `&'<path>' <value>` (PATH anchor — docs/language/pointers/bookmarks;
...
class Parser {
  ...
  object(): Mapping {
    ...
    for (;;) {
      ...
      if (back && this.peek() === '*') {
        // `~*'…'` — a KEYLESS back member (reverse positional membership, docs/language/pointers/bookmarks):
