...
`body.yo` holds the **data**, `meta.yo` holds **type/format** metadata; a plain
...
No `.yo/` overlay at all — each file's `(type, format)` is inferred from its recognized
...
| # | example | concrete / shows |
|---|---------|------------------|
| 67 | [`67-pdf-tags`](67-pdf-tags)   | real PDFs (messy filenames) + a `body.yo` overlay: a `!!<*yamlover/$defs/tag>` taxonomy whose slugs link papers by `*` pointer, with each paper mirroring `~slug:` back — membership authored both ways, reconciled by the engine |
...
type or format above is either inferred from an extension, declared in a `meta.yo`, or
